...
async def test_form_success(hass: HomeAssistant, bypass_get_data) -> None:
    ...
    with patch("custom_components.nextenergy_battery.async_setup_entry", return_value=True):
        result2 = await hass.config_entries.flow.async_configure(result["flow_id"], MOCK_CONFIG)
        await hass.async_block_till_done()

    assert result2["type"] == FlowResultType.CREATE_ENTRY

async def test_options_flow(hass: HomeAssistant, bypass_get_data) -> None:
