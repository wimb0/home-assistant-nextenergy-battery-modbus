...
async def test_sensor_values(hass: HomeAssistant, mock_modbus_client):
...
async def test_charging_scenario(hass: HomeAssistant, mock_modbus_client):
    """Test sensor values for a charging scenario."""
    entry = MockConfigEntry(domain=DOMAIN, data=MOCK_CONFIG, options=MOCK_CONFIG)
    entry.add_to_hass(hass)

    mock_modbus_client.read_sensors.side_effect = (
        lambda keys: MOCK_STATIC_DATA
        if "model_name" in keys
        else MOCK_DYNAMIC_DATA_CHARGE
    )
...
async def test_unavailable_scenario(hass: HomeAssistant, mock_modbus_client):
    ...
    mock_modbus_client.read_sensors.side_effect = Exception("API is down")

    # Force the time forward to trigger the next update
...
async def test_disabled_sensors(hass: HomeAssistant, mock_modbus_client):
